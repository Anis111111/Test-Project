...
def update_project(request , pk):
    ...
    project.descripthion = request.data['descripthion']
    project.category = request.data['category']
    project.rating = request.data['rating']
...
def add_review(request,pk):
    ...
    if data['rating'] <= 0 or data['rating'] > 10 :
        return Response({"error":"Please select between 1 to 10 only"},
...
def delete_review(request,pk):
    ...
    if review.exists() :
        ...
        if rating['avg_ratings'] is None:
            ...
            project.save()
            return Response(
                            {"details":"Project review Deleted !!!"}
                            )
    else:
        return Response(
